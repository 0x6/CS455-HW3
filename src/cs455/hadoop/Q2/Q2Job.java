...
public class Q2Job {
    public static class Q2Mapper extends Mapper<LongWritable, Text, Text, PairWritable> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (new Integer(value.toString().substring(24, 28)) == 1) {
                ...
                long population = new Long(value.toString().substring(300, 309));

                long mNeverMarried = new Long(value.toString().substring(4422, 4431));
                long fNeverMarried = new Long(value.toString().substring(4467, 4476));

                context.write(new Text(state + " M"), new PairWritable(population, mNeverMarried));
                context.write(new Text(state + " F"), new PairWritable(population, fNeverMarried));
            }
        }
    }

    public static class Q2Reducer extends Reducer<Text, PairWritable, Text, Text> {
        @Override
        protected void reduce(Text key, Iterable<PairWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for(PairWritable val : values){
                ...
                neverMarried += val.getSecond();
            }

            context.write(key, new Text( (double)neverMarried/population + "" ));
        }
    }
